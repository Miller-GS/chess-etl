    ...
    def _process_moves(self, game: chess.pgn.Game) -> list:
        ...
        for move in game.mainline_moves():
            move_dict = {
                ...
                'moved_unit': board.piece_at(move.from_square).symbol() if board.piece_at(move.from_square) else None,
                'captured_unit': board.piece_at(move.to_square).symbol() if board.piece_at(move.to_square) else None,
                'promoted_unit': None,
                'uci': move.uci(),
                'san': board.san(move),
            ...
            if move_dict['is_promotion']:
                move_dict['promoted_unit'] = chess.PIECE_SYMBOLS[move.promotion].upper() if board.turn == chess.WHITE else chess.PIECE_SYMBOLS[move.promotion].lower()
